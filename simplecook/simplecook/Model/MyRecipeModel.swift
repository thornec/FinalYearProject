...
struct MyRecipeModel : Identifiable, Codable {
    ...
    // initalizer
    init(id: UUID = UUID(), title: String, ingredients: [String], method: [String], cost: Int, cooktime: Int, isCustom: Bool, isSaved: Bool) {
            // default values
            self.id = id
            self.title = title
...
extension MyRecipeModel {
    ...
    // return value
    var data : Data {
        return Data(title:title, ingredients:ingredients, method:method, cost:cost, cooktime:cooktime, isCustom:isCustom, isSaved:isSaved)
    }
